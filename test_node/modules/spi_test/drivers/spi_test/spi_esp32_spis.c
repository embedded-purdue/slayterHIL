...
static int configure_slave_cs(struct spi_context *ctx)
{
	int ret;
	const struct gpio_dt_spec *cs_gpio = ctx->cs_gpios;

	if (ctx->num_cs_gpios > 1)
	{
		LOG_ERR("Multiple CS lines not supported for slave peripheral configuration");
		return -EINVAL;
	}
...
#define ESP32_SPIS_INIT(idx)                                                                                                                                                                                        \
	...
	static struct spi_esp32_data spi_data_##idx = {                                                                                                                                                                 \
		...
		.dev_config = {.half_duplex = DT_INST_PROP(idx, half_duplex), GET_AS_CS(idx).positive_cs = DT_INST_PROP(idx, positive_cs), .no_compensate = DT_INST_PROP(idx, dummy_comp), .sio = DT_INST_PROP(idx, sio)}}; \
                                                                                                                                                                                                                    \
